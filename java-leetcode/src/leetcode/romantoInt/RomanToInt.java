 ...
 *      给定一个罗马数字，将其转换成整数。输入确保在 1 到 3999 的范围内。MMM + CM  + XC  + IX
 *      例子：
...
public class RomanToInt {
    ...
    public static void main(String[] args){
        ...
        System.err.println(romanToInt.romanToInt("MMMCMXCIX"));
    }
    ...
    public  int romanToInt(String s){
        ...
        Map<String,Integer> map = this.initData();
        for(int i=0;i<length;i++){
            StringBuffer buffer = new StringBuffer();
            if (i < length-1){
                buffer.append(chars[i]).append(chars[i+1]);
            }else{
                buffer.append(chars[i]);
            }
            if (map.containsKey(buffer.toString())){
                // 存在两个组合的情况
                result+=map.get(buffer.toString());
                i++;
            }else{
